...
global.socialowner   = "TIKTOK: @zyyzall, INSTAGRAM: @abcdeezall";

// ─── BANNER NAMA 3D (ANSI shadow effect) ───────────────────────────────────
function print3DBanner() {
  const name = global.namabot;
  const line = "═".repeat(Math.max(20, name.length + 8));
  console.log("\x1b[36m" + line + "\x1b[0m");
  console.log("\x1b[90m   " + name + "\x1b[0m"); // bayangan (shadow) - offset bawah-kanan
  console.log("\x1b[1m\x1b[35m " + name + "\x1b[0m"); // layer utama (depan)
  console.log("\x1b[1m\x1b[33m" + name + "\x1b[0m"); // highlight atas-kiri biar terlihat 3D
  console.log("\x1b[36m" + line + "\x1b[0m");
}

// ─── LOG CHAT KE TERMINAL ───────────────────────────────────────────────────
function logChat({ jid, sender, isGroup, text, isCmd }) {
  ...
  console.log(
    `\x1b[90m[${time}]\x1b[0m ${tag} \x1b[36m(${tipe})\x1b[0m \x1b[33m${nomor}\x1b[0m ➜ ${text}`
  );
}
...
async function handleSmeme(sock, msg, jid, text) {
  ...
  try {
    ...
    const memeBuffer = await createMemeLocal(imgBuffer, topText, bottomText);

    if (!memeBuffer) {
      await react("❌");
      return reply(sock, msg, "❌ Gagal membuat meme. Pastikan *ffmpeg* sudah terinstall (pkg install ffmpeg) dan gambar yang dikirim valid.");
    }
...
async function handleMessage(sock, msg) {
  try {
    ...
    // 📋 Log setiap chat/command yang masuk ke terminal
    logChat({ jid, sender, isGroup, text, isCmd });
